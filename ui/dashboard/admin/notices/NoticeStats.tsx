...
const NoticeStats = () => {
  const { notices } = useAllNotices({}, 1, 100);
  const global = notices?.filter((n: any) => n.isGlobal)?.length || 0;
  const stats = [
    { label: "Total Notices", count: notices?.length || 0, icon: Bell, color: "from-cyan-400 to-blue-500" },
    { label: "Global", count: global, icon: Globe, color: "from-green-400 to-emerald-500" },
  ];
  return (
