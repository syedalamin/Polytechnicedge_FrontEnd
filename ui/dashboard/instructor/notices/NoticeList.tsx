...
const NoticeList = () => {
  ...
  const limit = 10;
  const { notices, meta, loading } = useAllNotices({}, page, limit);
  ...
  const columns: TableColumn<any>[] = [
    ...
    },
    {
      header: "Author",
      className: "px-4",
      accessor: (n) => <Text variant="body" color="dimmed" size="sm">{n.author?.email || "N/A"}</Text>,
    },
    {
      header: "Date",
      className: "px-4",
  ...
  return (
    <GlassCard paddingSize="md">
      <div className="p-4 sm:p-5 md:p-6 border-b border-white/5">
        <Text color="white" variant="body" size="md">All Notices</Text>
      </div>
      <GridTable
        data={notices}
        columns={columns}
        rowKeyAccessor="id"
        gridLayoutClass="grid-cols-[2fr_1fr_1.5fr_1.5fr]"
        isLoading={loading}
