...
const ModuleList = () => {
  ...
  const columns: TableColumn<any>[] = [
    ...
    },
    {
      ...
      className: "px-4",
      accessor: (m) => (
        <Text variant="body" color="dimmed" size="sm">{m.contents?.length || 0} items</Text>
      ),
    },
    {
      header: "Quizzes",
      className: "px-4",
      accessor: (m) => (
        <Text variant="body" color="dimmed" size="sm">{m.quizzes?.length || 0} quizzes</Text>
      ),
    },
    {
      header: "Actions",
      ...
      accessor: (m) => (
        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <Link href={`/student/courses/${courseId}/modules/${m.id}`}>
