...
type InstructorFormData = z.infer<
  typeof instructorSchema.createInstructorSchema
>;

const CreateInstructorModal = ({ refetch }: CreateInstructorModalProps) => {
  const dispatch = useAppDispatch();
  const isOpen = useAppSelector(
    (state: any) => !!state.modal?.["addInstructor"],
  );
  ...
  const onSubmit = async (data: InstructorFormData) => {
    ...
    try {
      ...
      const errorMsg =
        err?.data?.message || err?.data || "Failed to create instructor";
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={() => dispatch(closeModal("addInstructor"))}
