...
import { Video, FileText, Link as LinkIcon, Lock, FileQuestion } from "lucide-react";
...
const ContentList = () => {
  ...
  const { quizzes, loading: quizLoading } = useQuizzesByModuleId(moduleId);
  ...
  const columns: TableColumn<any>[] = [
    ...
    },
    {
      ...
      className: "px-4",
      accessor: (c) => (
        <Text variant="body" color="dimmed" size="sm">{c.duration ? `${c.duration} min` : "N/A"}</Text>
      ),
    },
    {
      header: "Status",
